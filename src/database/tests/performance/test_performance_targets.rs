...
use ai_core_database::{
    connections::clickhouse::{ApiRequest, SystemMetric, WorkflowEvent},
    connections::{ClickHouseConfig, MongoConfig, PostgresConfig, RedisConfig},
    DatabaseConfig, DatabaseError, MonitoringConfig,
};
...
async fn test_postgresql_performance_targets() {
    ...
    assert!(
        simple_query_duration.as_millis() < 10,
        "Simple query took {}ms, expected < 10ms",
        simple_query_duration.as_millis()
    );
    ...
    assert!(
        transaction_duration.as_millis() < 100,
        "Complex transaction took {}ms, expected < 100ms",
        transaction_duration.as_millis()
    );
    ...
    println!(
        "✅ PostgreSQL performance: {} took {}ms (target: {}ms)",
...
async fn test_clickhouse_performance_targets() {
    ...
    assert!(
        ...
        "Analytics query took {}ms, expected < 1000ms",
        analytics_duration.as_millis()
    );
    ...
    let _serialized_bulk: Vec<_> = bulk_events
        .iter()
    ...
    assert!(
        throughput > 10000.0,
        "Bulk insertion throughput was {:.1} events/sec, expected > 10k/sec",
        throughput
    );

    println!(
        "✅ ClickHouse performance: Analytics query {}ms, bulk throughput {:.1} events/sec",
        analytics_duration.as_millis(),
        throughput
    );
}
...
async fn test_mongodb_performance_targets() {
    ...
    assert!(
        doc_ops_duration.as_millis() < 50,
        "Document operations took {}ms, expected < 50ms",
        doc_ops_duration.as_millis()
    );
    ...
    assert!(
        aggregation_duration.as_millis() < 500,
        "Aggregation took {}ms, expected < 500ms",
        aggregation_duration.as_millis()
    );

    println!(
        "✅ MongoDB performance: Document ops {}ms, aggregation {}ms",
        doc_ops_duration.as_millis(),
        aggregation_duration.as_millis()
    );
}
...
async fn test_redis_performance_targets() {
    ...
    assert!(
        cache_ops_duration.as_micros() < 1000, // < 1ms = 1000 microseconds
        "Cache operations took {}μs, expected < 1000μs",
        cache_ops_duration.as_micros()
    );
    ...
    let _batch_serialized: Vec<_> = batch_operations
        .iter()
    ...
    assert!(
        complex_ops_duration.as_millis() < 10,
        "Complex operations took {}ms, expected < 10ms",
        complex_ops_duration.as_millis()
    );
    ...
    assert!(
        pubsub_duration.as_millis() < 5,
        "Pub/sub operations took {}ms, expected < 5ms",
        pubsub_duration.as_millis()
    );

    println!(
        "✅ Redis performance: Cache ops {}μs, complex ops {}ms, pub/sub {}ms",
        cache_ops_duration.as_micros(),
        complex_ops_duration.as_millis(),
        pubsub_duration.as_millis()
    );
...
async fn test_cross_database_performance() {
    ...
    let events: Vec<_> = (0..100)
        ...
        .collect();
    let _events_serialized: Vec<_> = events
        .iter()
        .map(|e| serde_json::to_string(e).unwrap())
        .collect();
    ...
    for (db_name, duration) in &operation_results {
        match *db_name {
            "PostgreSQL" => assert!(
                *duration < 10,
                "PostgreSQL took {}ms, expected < 10ms",
                duration
            ),
            "ClickHouse" => assert!(
                *duration < 100,
                "ClickHouse took {}ms, expected < 100ms",
                duration
            ),
            "MongoDB" => assert!(
                *duration < 25,
                "MongoDB took {}ms, expected < 25ms",
                duration
            ),
            "Redis" => assert!(
                *duration < 1000,
                "Redis took {}μs, expected < 1000μs",
                duration
            ), // microseconds
            _ => {}
        }
    ...
    assert!(
        overall_duration.as_millis() < 150,
        "Cross-database coordination took {}ms, expected < 150ms",
        overall_duration.as_millis()
    );

    println!(
        "✅ Cross-database performance: Overall {}ms",
        overall_duration.as_millis()
    );
    for (db_name, duration) in operation_results {
        if db_name == "Redis" {
...
async fn test_performance_under_load() {
    ...
    // Analyze results
    let operation_times: Vec<u128> = results.into_iter().map(|r| r.unwrap().1).collect();
    ...
    assert!(
        avg_time < 20.0,
        "Average operation time {}ms under load, expected < 20ms",
        avg_time
    );
    assert!(
        max_time < 50,
        "Maximum operation time {}ms under load, expected < 50ms",
        max_time
    );
    assert!(
        load_test_duration.as_millis() < 1000,
        "Load test took {}ms, expected < 1000ms",
        load_test_duration.as_millis()
    );
    ...
    assert!(
        throughput > 10.0,
        "Throughput {:.1} ops/sec under load, expected > 10 ops/sec",
        throughput
    );

    println!(
        "✅ Performance under load: {} concurrent ops in {}ms",
        operation_times.len(),
        load_test_duration.as_millis()
    );
    println!(
        "   Average: {:.1}ms, Min: {}ms, Max: {}ms, Throughput: {:.1} ops/sec",
...
async fn test_memory_performance() {
    ...
    let _serialized_dataset: Vec<_> = large_dataset
        .iter()
    ...
    assert!(
        dataset_creation_time.as_millis() < 1000,
        "Dataset creation took {}ms, expected < 1000ms",
        dataset_creation_time.as_millis()
    );

    assert!(
        serialization_time.as_millis() < 2000,
        "Serialization took {}ms, expected < 2000ms",
    ...
    assert!(
        items_per_sec > 5000.0,
        "Processing rate {:.1} items/sec, expected > 5000 items/sec",
        items_per_sec
    );

    println!(
        "✅ Memory performance: {} items in {}ms, processing rate: {:.1} items/sec",
        large_dataset.len(),
        dataset_creation_time.as_millis(),
        items_per_sec
    );
    println!(
        "   Serialization: {}ms for {} items",
        serialization_time.as_millis(),
        large_dataset.len()
    );
}
...
async fn test_error_handling_performance() {
    ...
    assert!(
        avg_error_time < 100.0, // < 100 microseconds average
        "Average error handling time {:.1}μs, expected < 100μs",
        avg_error_time
    );

    assert!(
        max_error_time < 1000, // < 1ms maximum
        "Maximum error handling time {}μs, expected < 1000μs",
        max_error_time
    );

    assert!(
        total_error_test_time.as_millis() < 100,
        "Total error test time {}ms, expected < 100ms",
        total_error_test_time.as_millis()
    );

    println!(
        "✅ Error handling performance: {} errors in {}ms",
        error_results.len(),
        total_error_test_time.as_millis()
    );
    println!(
        "   Average: {:.1}μs, Max: {}μs per error",
