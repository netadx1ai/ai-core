...
mod shared_performance_tests {
    ...
    use ai_core_database::{DatabaseError, connections::PostgresConfig};
    ...
    fn test_performance_measurement_accuracy() {
        ...
        assert!(elapsed.as_millis() >= 8 && elapsed.as_millis() <= 15,
               "Timing measurement inaccurate: {}ms", elapsed.as_millis());
    }
    ...
    fn test_performance_data_structures() {
        ...
        assert!(serialization_time.as_micros() < 1000,
               "Performance metric serialization too slow: {}μs", serialization_time.as_micros());
    }
    ...
    async fn test_baseline_operation_performance() {
        ...
        for (operation_name, operation) in operations {
            ...
            assert!(avg_time_ns < 1000,
                   "{} average time {}ns, expected < 1000ns", operation_name, avg_time_ns);
    ...
    async fn test_async_operation_performance() {
        ...
        assert!(total_time.as_millis() < 50,
               "Concurrent async operations took {}ms, expected < 50ms", total_time.as_millis());

        println!("✅ Async performance: 100 concurrent operations in {}ms", total_time.as_millis());
    }
    ...
    fn test_memory_allocation_performance() {
        ...
        assert!(allocation_time.as_millis() < 100,
               "Memory allocation took {}ms, expected < 100ms", allocation_time.as_millis());
        ...
        println!("✅ Memory allocation: 10k configs in {}ms", allocation_time.as_millis());
    }
    ...
    fn test_performance_regression_detection() {
        ...
        for (operation_name, baseline_duration) in baseline_operations {
            ...
            assert!(actual_duration <= max_allowed,
                   ...
                   operation_name, actual_duration.as_micros(), baseline_duration.as_micros(), max_allowed.as_micros());

            println!("✅ Performance check {}: {}μs (baseline: {}μs)",
                    operation_name, actual_duration.as_micros(), baseline_duration.as_micros());
        }
    }
    ...
    fn test_performance_monitoring_utilities() {
        ...
        assert!(duration.as_millis() >= 4 && duration.as_millis() <= 10,
               "Measurement utility inaccurate: {}ms", duration.as_millis());

        // Test with database operations
        let (config, config_duration) = measure_operation(|| PostgresConfig::default());

        assert!(config_duration.as_micros() < 1000,
               "Config creation too slow: {}μs", config_duration.as_micros());
    ...
    fn test_performance_targets() {
        ...
        let performance_targets = vec![
            ...
            ("PostgreSQL", "complex_transaction", Duration::from_millis(100)),
        ...
        for (database, operation, target) in performance_targets {
            ...
            assert!(target > Duration::from_nanos(1),
                   "Performance target too aggressive for {} {}", database, operation);

            assert!(target < Duration::from_secs(10),
                   "Performance target too lenient for {} {}", database, operation);

            println!("✅ Performance target: {} {} < {}ms",
                    database, operation, target.as_millis());
        }
    }
}
